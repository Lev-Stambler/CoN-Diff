...
\begin{figure}[H]
	\begin{mdframed}
		...
		Aggregation for time step $t$: \begin{itemize}
			\item Each party $i$ calls $\vec{r}_i = F(s_i, t)$  where $F$ is the HPRF
			\item Each party generates noise $\vec{x}_i$ with standard deviation $O(1/n)$ and publishes data $\overrightarrow{ct}_i = \vec{v}_i + \vec{r}_i + \vec{x}_i$ as well as a proof that (1) $\vec{r}_i$ is generated with key $s_i$ relative to $Comm(s_i)$ (2) $\vec{r}_i = F(s_i, t)$ and (3) $\vec{v}_i + \vec{x}_i$ are within some bounds specified by the protocol
			\item To decode the data, a third party checks all the proofs and does $\sum_i \overrightarrow{ct}_i - F(s_{\sum}, t) = \sum_i \vec{v}_i + \sum \vec{x}_i$
		\end{itemize}
	\end{mdframed}
